...
class TokenAggregate:
    ...
    def generate_aggregate_list(
        ...
        while current_clear_token_id<len(tokens_clear) or current_spacy_token_id<len(spacy_tokens):
            if(current_clear_token_id>len(tokens_clear)) or current_spacy_token_id>len(spacy_tokens):
                break
            ...
            if( len(constructed_model_token)< len(constructed_spacy_token)):
                ...
                spacy_token_to_current_model_tokens.append(spacy_tokens[current_spacy_token_id])
                current_spacy_token_id +=1
