...
def generate_aggregates(
    ...
    for id,doc in enumerate(docs):
        ...
        spacy_token_to_our_tokens = TokenAggregate.generate_aggregate_list(doc,exps[id][PREDICTED_CLASS],tokens_clear,tokens)
        if spacy_token_to_our_tokens is not False:
            all_aggregates = all_aggregates + spacy_token_to_our_tokens
    
    return all_aggregates

def transform_aggregate_into_mapping(aggregate: TokenAggregate)->list[(str,float,str)]:
    list: list[(str,float,str)] = []
...
def tokenize_using_cuda(obs:str,
                        pipeline:TextClassificationPipeline
                        ) ->torch.Tensor:
